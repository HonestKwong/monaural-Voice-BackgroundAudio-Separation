...
def predict_DRNN_batch():
    file_path = "./MIR-1K/Mix_Full"
    # file_path = "./MIR-1K/recognize/mix"
    ...
    for wav in filelist[-20:]:
        ...
        print ("DRNN分离音乐", Music_name, "耗时：", time_flies,"s")
        time_sum.append(time_flies)
    time_average = np.mean(time_sum)
    print ("DRNN分离音乐平均耗时：", time_average,"s")

def predict_Unet_CNN_batch():
    # file_path = "./MIR-1K/Mix_Full"
    file_path = "./MIR-1K/recognize/mix"
    filelist = librosa.util.find_files(file_path,ext="wav")
    ...
    if loss_type == "MSE":
        ...
        if not os.path.exists("./outputs/UnetCNN_outputs/L1_recognize/accompaniment"):
            os.mkdir("./outputs/UnetCNN_outputs/L1_recognize/accompaniment")
        if not os.path.exists("./outputs/UnetCNN_outputs/L1_recognize/vocals"):
            os.mkdir("./outputs/UnetCNN_outputs/L1_recognize/vocals")
        tensorboard_directory = './model_cpk/model_CNN/L1_recognize/tensorboard'
        clear_tensorboard = False
        model_directory = './model_cpk/model_CNN/L1_recognize'
    ...
    for wav in filelist[-20:]:
        ...
        if loss_type == "MSE":
            ...
            vocal_path = os.path.join('.', 'outputs', 'UnetCNN_outputs','L1_recognize', "vocals" , Music_name+'_vocal.wav')
            music_path = os.path.join('.', 'outputs', 'UnetCNN_outputs','L1_recognize', "accompaniment" , Music_name+'_music.wav')         
        ...
        time_flies = (time.clock() - time_start)        
        print ("CNN分离音乐", Music_name, "耗时：", time_flies,"s")
        time_sum.append(time_flies)
    time_average = np.mean(time_sum)
    print ("CNN分离音乐平均耗时：", time_average,"s")

def predict_CNN_1D_batch():
    # file_path = "./MIR-1K/Mix_Full"
    file_path = "./MIR-1K/recognize/mix"
    filelist = librosa.util.find_files(file_path,ext="wav")
    ...
    if loss_type == "MSE":
        ...
        if not os.path.exists("./outputs/CNN_1D_outputs/MSE_SIR_recognize/accompaniment"):
            os.mkdir("./outputs/CNN_1D_outputs/MSE_SIR_recognize/accompaniment")
        if not os.path.exists("./outputs/CNN_1D_outputs/MSE_SIR_recognize/vocals"):
            os.mkdir("./outputs/CNN_1D_outputs/MSE_SIR_recognize/vocals")
        tensorboard_directory = './model_cpk/model_CNN_1D/MSE_SIR_recognize/tensorboard'
        clear_tensorboard = False
        model_directory = './model_cpk/model_CNN_1D/MSE_SIR_recognize'
    ...
    for wav in filelist[-20:]:
        ...
        if loss_type == "MSE":
            ...
            vocal_path = os.path.join('.', 'outputs', 'CNN_1D_outputs','MSE_SIR_recognize', "vocals" , Music_name+'_vocal.wav')
            music_path = os.path.join('.', 'outputs', 'CNN_1D_outputs','MSE_SIR_recognize', "accompaniment" , Music_name+'_music.wav')        
    ...
    time_average = np.mean(time_sum)
    print ("CNN_1D分离音乐平均耗时：", time_average,"s")

def predict_DRNN_outputs_Batch():
    # file_path = "./MIR-1K/Mix_Full"
    file_path = "./MIR-1K/recognize/mix"
    filelist = librosa.util.find_files(file_path,ext="wav")
    ...
    if loss_type == "MSE":
        ...
        if not os.path.exists("./outputs/RNN_rectified_outputs/MSE_SIR_recognize/accompaniment"):
            os.mkdir("./outputs/RNN_rectified_outputs/MSE_SIR_recognize/accompaniment")
        if not os.path.exists("./outputs/RNN_rectified_outputs/MSE_SIR_recognize/vocals"):
            os.mkdir("./outputs/RNN_rectified_outputs/MSE_SIR_recognize/vocals")
        tensorboard_directory = './model_cpk/model_RNN_outputs/MSE_SIR_recognize/tensorboard'
        clear_tensorboard = False
        model_directory = './model_cpk/model_RNN_outputs/MSE_SIR_recognize'
    ...
    for wav in filelist[-20:]:
        ...
        model = DRNN_Output_rectified(num_features = n_fft // 2 + 1, num_hidden_units = num_hidden_units, tensorboard_directory = tensorboard_directory, clear_tensorboard = clear_tensorboard, loss_type=loss_type)
        model.load_predict(file_dir = model_filepath)
        mix_wav_mag1, mix_wav_mag2, mix_wav_phase, audio_length = utils.LoadAudio(Audio_path)
        X = mix_wav_mag1
        X_width = np.array(X).shape[1]
        ...
        if loss_type == "MSE":    
            ...
            vocal_path = os.path.join('.', 'outputs', 'RNN_rectified_outputs','MSE_SIR_recognize', "vocals" , Music_name+'_vocal.wav')
            music_path = os.path.join('.', 'outputs', 'RNN_rectified_outputs','MSE_SIR_recognize', "accompaniment" , Music_name+'_music.wav')
            
        utils.SaveAudio(vocal_path, Mag_vox[:,:,0],mix_wav_phase, audio_length=audio_length)
        utils.SaveAudio(music_path, Mag_acc[:,:,0],mix_wav_phase, audio_length=audio_length)
        time_flies = (time.clock() - time_start)        
        print ("DRNN输出修改分离音乐", Music_name, "耗时：", time_flies,"s")
        time_sum.append(time_flies)
    time_average = np.mean(time_sum)
    print ("DRNN输出修改分离音乐平均耗时：", time_average,"s")

if __name__ == "__main__":
    # predict_DRNN_batch()
    # predict_Unet_CNN_batch()
    # predict_CNN_1D_batch()
    predict_DRNN_outputs_Batch()
